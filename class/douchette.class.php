...
class Tdouchette extends TObjetStd
{
	/**
	 * Draft status
	 */
	...
	public function __construct()
	{
		...
		$this->set_table(MAIN_DB_PREFIX.'douchette');
...
//		$this->setChild('TdouchetteChild','fk_douchette');
		...
		$this->generic->element = 'douchette';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$numero = get_next_value($db, $mask, 'douchette', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showdouchette") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/douchette/card.php', 1).'?id='.$this->getId(). $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='douchetteStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='douchetteStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='douchetteStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='douchetteStatusAccepted'; $shortkeytrans='Accepted'; }
 ...
 */
class Douchette extends Tdouchette
{
	private $PDOdb;
...
/*
class TdouchetteChild extends TObjetStd
{
	public function __construct()
	{
		$this->set_table(MAIN_DB_PREFIX.'douchette_child');
		
		$this->add_champs('fk_douchette', array('type' => 'integer', 'index' => true));
