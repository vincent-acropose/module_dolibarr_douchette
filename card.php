...
$hookmanager->initHooks(array('douchettecard', 'globalcard'));
...
//			}
			...
			header('Location: '.dol_buildpath('/douchette/card.php', 1).'?id='.$object->getId());
			exit;
		...
		case 'confirm_clone':
			...
			break;
		case 'modif':
			if (!empty($user->rights->douchette->write)) $object->setDraft($PDOdb);
				
			break;
		case 'confirm_validate':
			if (!empty($user->rights->douchette->write)) $object->setValid($PDOdb);
			
			header('Location: '.dol_buildpath('/douchette/card.php', 1).'?id='.$object->getId());
			exit;
			break;
		case 'confirm_delete':
			if (!empty($user->rights->douchette->write)) $object->delete($PDOdb);
			
			header('Location: '.dol_buildpath('/douchette/list.php', 1));
			exit;
			break;
...
{
	load_fiche_titre($langs->trans("Newdouchette"));
	dol_fiche_head();
}
else
{
...
if ($mode == 'edit') echo $formcore->begin_form($_SERVER['PHP_SELF'], 'form_douchette');

$linkback = '<a href="'.dol_buildpath('/douchette/list.php', 1).'">' . $langs->trans("BackToList") . '</a>';
print $TBS->render('tpl/card.tpl.php'
	...
	,array(
		...
		,'view' => array(
			'mode' => $mode
			,'action' => 'save'
			,'urlcard' => dol_buildpath('/douchette/card.php', 1)
			,'urllist' => dol_buildpath('/douchette/list.php', 1)
...
//			,'showNote' => $formcore->zonetexte('', 'note', $object->note, 80, 8)
		...
		,'conf' => $conf
		,'Tdouchette' => array(
			'STATUS_DRAFT' => Tdouchette::STATUS_DRAFT
			,'STATUS_VALIDATED' => Tdouchette::STATUS_VALIDATED
			,'STATUS_REFUSED' => Tdouchette::STATUS_REFUSED
			,'STATUS_ACCEPTED' => Tdouchette::STATUS_ACCEPTED
		)
	)
);
